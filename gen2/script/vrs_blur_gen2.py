...
# ---------------------------------------------------------------------------
# Passthrough: combine blurred streams with original non-camera streams
# ---------------------------------------------------------------------------

_VRS_SEARCH_PATHS = [
    "vrs",  # PATH
    os.path.expanduser("~/bin/vrs"),
    os.path.expanduser("~/miniconda3/bin/vrs"),
]


def _find_vrs_cmd(explicit_path: Optional[str] = None) -> str:
    """Locate the ``vrs`` CLI binary.

    If *explicit_path* is given it is returned directly (caller is responsible
    for correctness).  Otherwise we search PATH and a few well-known locations.
    """
    if explicit_path:
        return explicit_path

    for candidate in _VRS_SEARCH_PATHS:
        found = shutil.which(candidate)
        if found is not None:
            return found

    raise FileNotFoundError(
        "Could not find 'vrs' CLI.  Install it (pip install vrs) or pass "
        "--vrs_cmd /path/to/vrs."
    )


def _run_vrs_cmd(cmd: List[str]) -> subprocess.CompletedProcess:
    """Run a vrs CLI command."""
    return subprocess.run(cmd, capture_output=True, text=True)


def _combine_with_passthrough(
    blurred_vrs_path: str,
    original_vrs_path: str,
    output_path: str,
    camera_stream_ids: List[str],
    vrs_cmd: str,
) -> None:
    """Combine blurred camera streams with non-camera streams from the original.

    VRS stream filters (``-`` exclude flags) apply globally to all input files,
    so we cannot exclude camera streams from only the original in a single
    command.  Instead we use two steps:

    1. Extract non-camera streams from the original into a temp file::

           vrs copy <original> --to <passthrough_tmp> - <sid1> - <sid2> ...

    2. Merge the blurred camera streams with the passthrough temp file::

           vrs copy <blurred> <passthrough_tmp> --to <output>
    """
    passthrough_tmp = output_path + ".passthrough_tmp.vrs"

    # Log blurred file contents before merge.
    blurred_reader = SyncVRSReader(blurred_vrs_path)
    logger.info(
        f"Blurred VRS: {len(blurred_reader.stream_ids)} streams, "
        f"{blurred_reader.n_records} records"
    )
    for sid in sorted(blurred_reader.stream_ids):
        info = blurred_reader.get_stream_info(sid)
        logger.info(
            f"  {sid} ({info.get('device_name', '?')}): "
            f"{info.get('data_records_count', 0)} data records"
        )
    blurred_reader.close()

    # Step 1: extract non-camera streams from original
    cmd1: List[str] = [vrs_cmd, "copy", original_vrs_path, "--to", passthrough_tmp]
    for sid in camera_stream_ids:
        cmd1 += ["-", sid]

    logger.info("Passthrough step 1: extracting non-camera streams")
    result = _run_vrs_cmd(cmd1)
    if result.returncode != 0:
        stderr = result.stderr.strip()
        raise RuntimeError(
            f"vrs copy (extract passthrough) failed (exit {result.returncode}):\n{stderr}"
        )

    # Step 2: merge blurred camera streams + passthrough non-camera streams
    cmd2: List[str] = [
        vrs_cmd,
        "copy",
        blurred_vrs_path,
    ...
    logger.info("Passthrough step 2: merging blurred + passthrough streams")
    try:
        result = _run_vrs_cmd(cmd2)
        if result.returncode != 0:
            stderr = result.stderr.strip()
            raise RuntimeError(
                f"vrs copy (merge) failed (exit {result.returncode}):\n{stderr}"
            )
    finally:
        if os.path.exists(passthrough_tmp):
            os.remove(passthrough_tmp)

    # Log combined output contents.
    output_reader = SyncVRSReader(output_path)
    logger.info(
        f"Combined output: {len(output_reader.stream_ids)} streams, "
        f"{output_reader.n_records} records -> {output_path}"
    )
    for sid in sorted(output_reader.stream_ids):
        info = output_reader.get_stream_info(sid)
        logger.info(
            f"  {sid} ({info.get('device_name', '?')}): "
            f"{info.get('data_records_count', 0)} data records"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=True,
        help="Process streams in parallel threads (default: enabled). "
        "Use --no-parallel for single-threaded sequential processing.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to the 'vrs' CLI binary (used to combine blurred camera "
        "streams with non-camera streams from the original). If not set, "
        "searches PATH, ~/bin/vrs, and ~/miniconda3/bin/vrs.",
    )
...
def _workflow_sequential(
    ...
    """Single-threaded processing loop with per-stream batched inference."""
    ...
    # Per-stream batch accumulators.
    # Each value: list of (timestamp, image_bgr, metadata_items) tuples.
    pending: Dict[str, List[Tuple[float, np.ndarray, List[Tuple[str, object]]]]] = {
        sid: [] for sid in target_sids
    }

    debug_enc = getattr(args, "debug_encoding_only", False)

    def _flush_stream(sid: str) -> None:
        """Detect + blur + encode + write a batch for one stream."""
        batch = pending[sid]
        if not batch:
            return

        is_rgb = _is_rgb_stream(sid)
        images_bgr = [item[1] for item in batch]

        if debug_enc:
            blurred_list = images_bgr
        else:
            blurred_list = _detect_and_blur_batch(
                images_bgr,
                face_detector,
                lp_detector,
        ...
        for (ts, _, md_items), blurred_bgr in zip(batch, blurred_list):
            h, w = blurred_bgr.shape[:2]
            ...
            stream.create_data_record(ts, data_md, encoded_arr)
            writer.flush_records(ts)
            frame_counts[sid] += 1
    ...
    data_reader = reader.filtered_by_fields(
        stream_ids=target_set, record_types={"data"}
    )
    with patch_instances(fields=PATCH_INSTANCES_FIELDS):
        for record in tqdm(
            data_reader,
            desc="Processing VRS frames",
            unit="frame",
            ...
            if sid not in decoders:
                decoders[sid] = H265Decoder(is_rgb=is_rgb)
            image_rgb = decoders[sid].decode_frame(raw_bytes)
            if image_rgb is None:
                continue
            ...
            pending[sid].append((record.timestamp, image_bgr, metadata_items))

            if len(pending[sid]) >= batch_size:
                _flush_stream(sid)

        # Flush remaining partial batches for all streams.
        for sid in target_sids:
            _flush_stream(sid)
    ...
    for sid, encoder in encoders.items():
        trailing = encoder.flush()
        if trailing:
            logger.info(f"Stream {sid}: flushed {len(trailing)} trailing bytes")
...
def workflow(args: argparse.Namespace) -> None:
    ...
    use_parallel = getattr(args, "parallel", True) and len(target_sids) > 1
    if use_parallel:
        frame_counts = _workflow_parallel(
            args, face_detector, lp_detector, reader, writer, streams, target_sids
        )
    else:
        if getattr(args, "parallel", True) and len(target_sids) == 1:
            logger.info("Only 1 stream — falling back to sequential processing")
        frame_counts = _workflow_sequential(
            args, face_detector, lp_detector, reader, writer, streams, target_sids
        )
    ...
    logger.info(
        f"Done ({mode_str}{passthrough_str}). {total_frames} frames across "
